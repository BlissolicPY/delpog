...
(() => {
  ...
  // Lizzy McAlpine — "the light in the painting" (3:53), the Official Video on
  // her own verified channel. Duration is NOT hard-coded: it comes from
  // getDuration() once the player is ready.
  const VIDEO_ID = "CEyHYCZAtOM";
  ...
  // dp2:, not dp: — the suffix is carried over from the sibling sites, where the
  // bare namespace held volumes on a pre-curve scale and a pause flag from
  // before autoplay existed. Keeping it means a stray dp: key can never be read
  // as if it were a position on the perceptual scale below.
  const store = {
    get(k, fallback) {
      try {
        const v = localStorage.getItem("dp2:" + k);
    ...
    set(k, v) {
      try {
        localStorage.setItem("dp2:" + k, String(v));
      } catch {
  ...
  // the stored "vol" is a slider position on the perceptual scale, never a raw
  // YouTube amplitude — see VOLUME_CURVE
  ...
  /* ---------- start ----------
     ...
     fallback, no getAutoplayPolicy() check, no data-blocked state. Audible
     playback after a click is simply allowed everywhere. Do not re-add a probe:
     with a guaranteed gesture there is nothing left to detect.
  ...
  window.addEventListener("dp:enter", () => {
    if (ready) enter();
