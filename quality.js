/* delpog.xyz — hardware-adaptive quality.
   ...
   "low", and fires `dp:quality` when that changes. CSS reads the attribute for
...
(() => {
  ...
  const set = (next) => {
    ...
    tier = next;
    root.dataset.q = tier;
    window.dispatchEvent(new CustomEvent("dp:quality", { detail: { tier } }));
  };
